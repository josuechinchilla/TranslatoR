# ============================================================================
# Germplasm Import tab module (self-contained).
#
# Renders the Blank template / Translation Guide / Converter panels for the
# Germplasm Import template and wires up the download + conversion handlers.
# Template configuration and field definitions come from app_data.R
# (TEMPLATES$germplasm). Helper functions live in utils.R.
# ============================================================================

#' Germplasm Import tab UI
#'
#' @param id Module id.
...
mod_germplasm_server <- function(id, lang) {
  shiny::moduleServer(id, function(input, output, session) {
    ...
    output$body <- shiny::renderUI({
      ...
      blank_title <- if (lang() == "EN") "Download Blank Germplasm Template" else "Descargar la plantilla de Germoplasma en blanco"
      shiny::fluidRow(
        shiny::column(
          width = 12,
          # --- Blank template ---
