# ============================================================================
# Ontology tab module (self-contained).
#
# Renders the Blank template / Translation Guide / Converter panels for the
# Ontology template and wires up the download + conversion handlers.
# Template configuration and field definitions come from app_data.R
# (TEMPLATES$ontology). Helper functions live in utils.R.
# ============================================================================
...
mod_ontology_server <- function(id, lang) {
  shiny::moduleServer(id, function(input, output, session) {
    ...
    output$body <- shiny::renderUI({
      ...
      blank_title <- if (lang() == "EN") "Download Blank Ontology Template" else "Descargar la plantilla de Ontología en blanco"
      shiny::fluidRow(
        shiny::column(
          width = 12,
          # --- Blank template ---
    ...
    output$dl <- shiny::downloadHandler(
      ...
      content = function(file) {
        ...
        # Ontology only: also translate controlled-vocabulary column VALUES
        # (Term Type, Status, Method Class, Scale Class) - free text is untouched.
        sheets <- translate_vocab_cols(sheets, find_data_sheet(cur, fields), fields, ont_vocab, direction)
        if (cur$type == "csv") {
