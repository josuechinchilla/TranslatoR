# ============================================================================
# Sample Submission tab module (self-contained).
#
# Renders the Blank template / Translation Guide / Converter panels for the
# Sample Submission template and wires up the download + conversion handlers.
# Template configuration and field definitions come from app_data.R
# (TEMPLATES$sample). Helper functions live in utils.R.
# ============================================================================

#' Sample Submission tab UI
#'
#' @param id Module id.
...
mod_sample_server <- function(id, lang) {
  shiny::moduleServer(id, function(input, output, session) {
    ...
    output$body <- shiny::renderUI({
      ...
      blank_title <- if (lang() == "EN") "Download Blank Sample Submission Template" else "Descargar la plantilla de Envío de Muestra en blanco"
      shiny::fluidRow(
        shiny::column(
          width = 12,
          # --- Blank template ---
